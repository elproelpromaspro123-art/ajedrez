(function initOpeningsModule(global) {
    ...
    function buildQueryTokenGroups(rawQuery) {
        ...
        const groups = tokens.map((token) => {
            ...
            const alias = EXPLORER_QUERY_ALIASES[token];
            if (alias) {
    ...
    function filterTreeRowsSync(rows, filters) {
        ...
        const popularityTier = String(safeFilters.frequencyTier || "all").toLowerCase();
        const successTier = String(safeFilters.resultTier || "all").toLowerCase();
        const queryMatcher = buildQueryTokenGroups(safeFilters.query || "");
